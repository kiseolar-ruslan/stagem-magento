...
class AbstractBlockTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetBlockHtml()
    {
        ...
        $blockFactory = \integration\framework\Magento\TestFramework\Helper\Bootstrap::getObjectManager()->get(
    ...
    protected function _createSampleBlocks(
        ...
        for ($i = 0; $i < $qty; $i++) {
            ...
            if ($layout) {
                ...
                $block = \integration\framework\Magento\TestFramework\Helper\Bootstrap::getObjectManager()->create($className);
                $block->setNameInLayout($name);
            }
            $blocks[] = $block;
            $names[] = $name;
